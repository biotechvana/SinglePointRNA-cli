...
load_checkInputs <- function( input, metadata ){
  ...
  if( !is.null( metadata ) ){
    if(! file.exists( file.path( metadata ) ) ){
      log_error( paste0("Metadata file '", metadata,  "' not found.\n") )
    }
  }
  return(inputFile )
...
load_add_cell_md <- function( inputData, md_table, cellGroups=NULL ){
  ...
  if( idMatch == 0 ){
    log_warn("Metadata and input cell IDs do not match. Discarding metadata.")
  } else {
    if( idMatch < ncol( inputData ) ){
      log_warn('Only ', idMatch, ' / ', ncol(inputData), ' cell IDs present in metadata.')
    } 
